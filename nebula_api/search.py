...
def get_album_data(service: str, api_call: callable):
    ...
    try:
        ...
        release_year = call_results['release_year']
        cover_art = call_results['cover_art']
        anchor = f'{emojis[service]} [{service}]({url})\n'
    except Exception as error:
        url = ''
...
def search_track(artist: str, track: str):
    ...
    # Search on Spotify
    spotify = get_track_data('Spotify',search_spotify_track(artist.replace("'",''),track.replace("'",'')))
    if artist_name == '' and track_name == '': 
        artist_name = spotify['artist']
        track_name = spotify['track']

    # Search on Apple Music
    apple_music = get_track_data('Apple Music', search_apple_music_track(artist.replace("'",''), track.replace("'",'')))
    if artist_name == '' and track_name == '': 
        artist_name = apple_music['artist']
        track_name = apple_music['track']

    # Search on YouTube Music
    youtube_music = get_track_data('YouTube Music', search_youtube_music_track(artist.replace("'",''), track.replace("'",'')))
    if artist_name == '' and track_name == '': 
        artist_name = youtube_music['artist']
        track_name = youtube_music['track']

    # Search on Deezer
    deezer = get_track_data('Deezer', search_deezer_track(spotify['artist'], spotify['track']))
    if artist_name == '' and track_name == '': 
        artist_name = deezer['artist']
        track_name = deezer['track']

    # Search on TIDAL
    tidal = get_track_data('TIDAL', search_tidal_track(spotify['artist'], spotify['track']))
    if artist_name == '' and track_name == '': 
        artist_name = tidal['artist']
        track_name = tidal['track']

    # Bandcamp
    bandcamp = get_track_data('Bandcamp', search_bandcamp_track(spotify['artist'], spotify['track']))
    if artist_name == '' and track_name == '': 
...
def search_album(artist: str, album: str):
    ...
    # Search on Spotify
    spotify = get_album_data('Spotify', search_spotify_album(artist.replace("'",''),album.replace("'",'')))
    if artist_name == '' and album_name == '': 
        artist_name = spotify['artist']
        album_name = spotify['album']

    # Search on Apple Music
    apple_music = get_album_data('Apple Music', search_apple_music_album(artist.replace("'",''), album.replace("'",'')))
    if artist_name == '' and album_name == '': 
        artist_name = apple_music['artist']
        album_name = apple_music['album']

    # Search on YouTube Music
    youtube_music = get_album_data('YouTube Music', search_youtube_music_album(artist.replace("'",''), album.replace("'",'')))
    if artist_name == '' and album_name == '': 
        artist_name = youtube_music['artist']
        album_name = youtube_music['album']

    # Search on Deezer
    deezer = get_album_data('Deezer',search_deezer_album(spotify['artist'], spotify['album']))
    if artist_name == '' and album_name == '': 
        artist_name = deezer['artist']
        album_name = deezer['album']
    
    # Search on TIDAL
    tidal = get_album_data('TIDAL', search_tidal_album(spotify['artist'], spotify['album']))
    if artist_name == '' and album_name == '': 
        artist_name = tidal['artist']
        album_name = tidal['album']

    # Bandcamp
    bandcamp = get_album_data('Bandcamp', search_bandcamp_album(spotify['artist'], spotify['album']))
    if artist_name == '' and album_name == '': 
        artist_name = bandcamp['artist']
